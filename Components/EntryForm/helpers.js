...
export const schema = yup.object().shape({
	title: yup
		.string()
		.max(60, 'Cannot exceed 60 letters!')
		.required('You must provide a Title!')
		.min(4, 'Cannot be less than 4 letters!'),
	date: yup.date().required('An Entry must belong to a Date!'),
	description: yup.string().max(72, 'Cannot exceed 72 letters!').min(4, 'Cannot be less than 4 letters!'),
	mood: yup.string().required('You must provide a Mood!').oneOf(['VERY_GOOD', 'GOOD', 'WELL', 'BAD', 'VERY_BAD']),
	images: yup
		.array()
		.nullable(true)
		.of(yup.object().shape({ url: yup.string(), name: yup.string() })),
	location: yup.object().nullable(true).shape({
		main: yup.string(),
